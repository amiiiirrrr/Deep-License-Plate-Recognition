...
sys.path.append('ocrPlate/ocr_up/src_up/')
from dataloader_up import DataLoaderIAM, Batch
from methods_up import Model, DecoderType
from preprocess_up import Preprocessor
from configuration_up import BaseConfig
...
class inferOcr:
    def __init__(self):
        ...
        self.model_up = Model(self.args, list(open(
    ...
    def infer_up(self, img) -> None:
        ...
        # print("######################3 here ###############", img.shape)
        assert img is not None
        img = self.preprocessor.process_img(img)

        batch = Batch([img], None, 1)
        recognized, probability = self.model_up.infer_batch(batch, True)
