...
from dataloader_down import Batch
...
class Model:
    ...
    def __init__(self, args,
        ...
        self.division = self.args.img_size / 32
        # Whether to use normalization over a batch or a population
        self.is_train = tf.compat.v1.placeholder(tf.bool, name='is_train')

        # input image batch
        self.input_imgs = tf.compat.v1.placeholder(
            tf.float32, shape=(self.args.batch_size, self.args.img_size, self.args.img_height))
        ...
        # initialize TF
        self.sess1, self.saver1 = self.setup_tf()
    ...
    def setup_cnn(self) -> None:
        ...
        feature_vals = [1, 32, 64, 128, 128, 256, 256]
        # stride_vals = pool_vals = [(2, 2), (2, 2), (1, 2), (1, 2), (1, 2)]
        stride_vals = pool_vals = [(2, 2), (2, 2), (1, 2), (1, 2), (1, 2), (2, 2)]
    ...
    def setup_rnn(self) -> None:
        """Create RNN layers."""
        # print("##############################", self.cnn_out_4d.shape)
        rnn_in3d = tf.squeeze(self.cnn_out_4d, axis=[2])


        # basic cells which is used to build RNN
        cells = [tf.compat.v1.nn.rnn_cell.LSTMCell(num_units=self.args.num_hidden, state_is_tuple=True) for _ in
                 range(2)]  # 2 layers
        ...
        kernel = tf.Variable(tf.random.truncated_normal([1, 1, self.args.num_hidden * 2, len(self.char_list) + 1], stddev=0.1))
    ...
    def setup_tf(self) -> Tuple[tf.compat.v1.Session, tf.compat.v1.train.Saver]:
        ...
        sess1 = tf.compat.v1.Session()  # TF session
        # with tf.compat.v1.Session(graph=loaded_graph) as sess:
        saver1 = tf.compat.v1.train.Saver(max_to_keep=1)  # saver saves model to file

        latest_snapshot = tf.train.latest_checkpoint(self.args.model_dir)  # is there a saved model?

        # if model must be restored (for inference), there must be a snapshot
        if self.must_restore and not latest_snapshot:
            raise Exception('No saved model found in: ' + self.args.model_dir)

        # load saved model if available
        if latest_snapshot:
            print('Init with stored values from ' + latest_snapshot)
            saver1.restore(sess1, latest_snapshot)
        else:
    ...
    def train_batch(self, batch: Batch) -> float:
        ...
        max_text_len = batch.imgs[0].shape[0] // self.division
        ...
        _, loss_val = self.sess1.run(eval_list, feed_dict)
    ...
    def infer_batch(self, batch: Batch, calc_probability: bool = False, probability_of_gt: bool = False):
        ...
        # evaluate model
        eval_res = self.sess1.run(eval_list, feed_dict)
        ...
        if calc_probability:
            ...
            loss_vals = self.sess1.run(eval_list, feed_dict)
    ...
    def save(self) -> None:
        """Save model to file."""
        self.snap_ID += 1
        self.saver1.save(self.sess1, '../model_down/snapshot', global_step=self.snap_ID)
