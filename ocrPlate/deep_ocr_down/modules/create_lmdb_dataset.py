...
def create_dataset():
    ...
    down_paths = os.listdir('./dataset/down')
    valid_percent = 0.1 
    valid_paths = down_paths[0:int(valid_percent * len(down_paths))]
    train_paths = down_paths[int(valid_percent * len(down_paths)):]
    for valid_paht in valid_paths:
        print(valid_paht)
        shutil.copyfile(f'./dataset/down/{valid_paht}', './dataset/valid/'+ valid_paht)
    for train_paht in train_paths:
        shutil.copyfile(f'./dataset/down/{train_paht}', './dataset/train/'+ train_paht)
